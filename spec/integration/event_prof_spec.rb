...
describe "event prof scenario" do
  ...
  specify "run and print" do
    ...
    run_command("bin/auto-test-prof --plan #{plan} --command #{command}") do |output|
      ...
      expect(output).to match(/Total time: 00:0\d\.\d{3} of 00:0\d\.\d{3} \(\d{2}\.\d+%\)/)
      expect(output).to include "Total events: 5"

      expect(output).to include "Top 5 slowest suites (by time):"
      expect(output).to match(/Something \(\.\/spec\/fixtures\/tests\.rb:25\) – 00:00\.\d{3} \(3 \/ 3\) of 00:0\d\.\d{3} \(\d{2}\.\d+%\)/)
      expect(output).to match(/Another something \(\.\/spec\/fixtures\/tests\.rb:41\) – 00:00\.\d{3} \(2 \/ 2\) of 00:0\d\.\d{3} \(\d{2}\.\d+%\)/)
    end
  end
  ...
  specify "run without print" do
    ...
    run_command("bin/auto-test-prof --plan #{plan} --command #{command}") do |output|
      ...
      expect(output).not_to match(/Total time: 00:0\d\.\d{3} of 00:0\d\.\d{3} \(\d{2}\.\d+%\)/)
      expect(output).not_to include "Total events: 5"
      expect(output).not_to include "Top 5 slowest suites (by time):"
      expect(output).not_to match(/Something \(\.\/spec\/fixtures\/tests\.rb:25\) – 00:00\.\d{3} \(3 \/ 3\) of 00:0\d\.\d{3} \(\d{2}\.\d+%\)/)
      expect(output).not_to match(/Another something \(\.\/spec\/fixtures\/tests\.rb:41\) – 00:00\.\d{3} \(2 \/ 2\) of 00:0\d\.\d{3} \(\d{2}\.\d+%\)/)
    end
  end
