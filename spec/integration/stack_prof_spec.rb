...
describe "stack prof scenario" do
  ...
  specify "run and print" do
    ...
    run_command("bin/auto-test-prof --plan #{plan} --command #{command}") do |output|
      ...
      expect(output).to include "Executing 'run' with profiler:stack_prof and options:{:sample=>100}"
      expect(output).to include "[TEST PROF INFO] StackProf (raw) enabled globally: mode – wall, target – suite"
